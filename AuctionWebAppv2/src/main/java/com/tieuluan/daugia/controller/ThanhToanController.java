...
public class ThanhToanController {
	...
	public String example(
			@RequestParam(value = "masp", required = false, defaultValue = "-1") Long masp,
			HttpSession session, Model model) throws UnknownHostException {
		if (masp == -1)
		...
		if (sp.getTinhtrangdaugia() == 2) {
			model.addAttribute("tieude", "Lỗi");
			model.addAttribute("noidung", "Sản phẩm này đã bán hết");
			model.addAttribute("web", web);
			return "thongbao";
		} else if (sp.getTinhtrangdaugia() == 3) {
			model.addAttribute("tieude", "Lỗi");
			model.addAttribute("noidung", "Sản phẩm này đã được thanh toán");
			model.addAttribute("web", web);
			return "thongbao";
		} else if (sp.getTinhtrangdaugia() == 0) {
			model.addAttribute("tieude", "Lỗi");
			model.addAttribute("noidung", "Sản phẩm này chưa kết thúc đấu giá");
			model.addAttribute("web", web);
			return "thongbao";
	...
	public String thanhtoan(HttpServletRequest request, Model model) {
		...
		if (json.equals("ok")) {
			...
			model.addAttribute("noidung", "Bạn đã thanh toán thành công");
			model.addAttribute("web", web);
			return "thongbao";
		} else {
			model.addAttribute("tieude", "Lỗi");
	...
	public String thanhtoanPOST(HttpServletRequest request, Model model) {
		HttpSession session = request.getSession();
		String masp = request.getParameter("masp");
		String sotaikhoan = request.getParameter("sotaikhoan");
		String web = Server.web;
